{
  ...
  in flakelib {
    ...
    legacyPackages = {
      r3-build-tools = { flex'lib, stdenvNoCC, fetchurl, unzip }: let
        owner = "flashflashrevolution";
        repo = "rCubed-build-tools";
        version = "2024-02-03";
        rev = "440eabcad7a2f09d0f1f4c77aa1dd896fed60c9e";
      in nixlib.recurseIntoAttrs {
        inherit owner repo version rev;
        runtimes = fetchurl {
          url = "https://github.com/${owner}/${repo}/raw/${rev}/Runtimes.zip";
          sha256 = "sha256-LFobVHQTnerVSGTL4zaAhUO1krQ1u7h1TjSgd5xpkPc=";
        };
        sdk-lite = fetchurl {
          url = "https://github.com/${owner}/${repo}/raw/${rev}/SDK-Lite.zip";
          sha256 = "255f5f4f8d26221e03c0e1ccaec6eb066ce3ac2624520d4a62e73518c31d83e7";
        };
      };
      adobe-air-sdk-r3 = { flex'apache-flex-sdk, r3-build-tools, unzip, mkFlexSetupHook }: flex'apache-flex-sdk.overrideAttrs (old: rec {
        pname = "${r3-build-tools.repo}-sdk-r3";
        inherit (r3-build-tools) version;
        name = "${pname}-${version}";

        src = r3-build-tools.sdk-lite;
        ...
        setupHook = mkFlexSetupHook pname (placeholder: rec {
          FLEX_HOME = "${placeholder "out"}/${old.sdkHome}";
          AIR_HOME = FLEX_HOME;
          PLAYERGLOBAL_HOME = "${FLEX_HOME}/frameworks/libs/player";
        });
      });
    ...
    devShells = {
      default = { outputs'devShells'r3 }: outputs'devShells'r3;
      ...
      r3 = { outputs'devShells'air, adobe-air-sdk-r3 }: outputs'devShells'air.override {
        apache-air-sdk = adobe-air-sdk-r3;
      };
