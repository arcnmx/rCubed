  ...
  in flakelib {
    ...
    legacyPackages = {
      ...
      in stdenv.mkDerivation {
        ...
        pname = "air-sdk";
        src = requireFile {
          url = "https://airsdk.harman.com/download/${version}";
          name = "AIRSDK_Linux.zip";
          sha256 = "fe770249ab16615e93477dca436b66eba515093e14c06ce617c966e021770252";
        };
        sourceRoot = ".";
        ...
        installPhase = ''
          ...
          install -d $out/lib/air
          mv ./* $out/lib/air/
          autoPatchelf $out/lib/air/lib/nai/bin/naip

          for f in $(cd $out/lib/air/bin; echo *); do
            makeWrapper $out/lib/air/bin/$f $out/bin/$f \
              --set-default AIR_DISTRO debian \
              --set-default AIR_HOME $out/lib/air
    ...
    defaultPackage = "r3air";
    devShells = {
      default = { mkShell, gnumake, air-sdk, writeShellScriptBin }: mkShell {
        AIR_HOME = "${air-sdk}/lib/air";
        nativeBuildInputs = [
          gnumake
          air-sdk
          (writeShellScriptBin "r3-ruffle" ''
            make debug -j &&
              nix shell nixpkgs#ruffle nixpkgs#gnome.zenity -c ruffle_desktop bin/develop/R3Air.swf --player-runtime air -Pruffle=1 "$@"
          '')
          (writeShellScriptBin "r3-ruffle-release" ''
            make release -j &&
              nix shell nixpkgs#ruffle nixpkgs#gnome.zenity -c ruffle_desktop bin/release/R3Air.swf --player-runtime air -Pruffle=1 "$@"
          '')
        ];
      };
      flex = { mkShell, gnumake, air-sdk, apache-flex-sdk, jre, writeShellScriptBin }: mkShell {
        # TODO: this still needs work; building fonts only currently works with air, and embedded noteskins fail for different reasons...
        AIR_HOME = "${air-sdk}/lib/air";
        nativeBuildInputs = [
          gnumake
          apache-flex-sdk
